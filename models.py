...
def seedData(app):
    ...
    addCat(
        db, "Condiments", "Sweet and savory sauces, relishes, spreads, and seasonings"
    )
    ...
    addProduct(
        db, "Chef Anton's Cajun Seasoning", 2, 2, "48 - 6 oz jars", 22.0000, 53, 0, 0, 0
    )
    ...
    addProduct(
        db, "Northwoods Cranberry Sauce", 3, 2, "12 - 12 oz jars", 40.0000, 6, 0, 0, 0
    )
    ...
    addProduct(
        db, "Queso Manchego La Pastora", 5, 4, "10 - 500 g pkgs.", 38.0000, 86, 0, 0, 0
    )
    ...
    addProduct(
        db, "Sir Rodney's Marmalade", 8, 3, "30 gift boxes", 81.0000, 40, 0, 0, 0
    )
    addProduct(
        db, "Sir Rodney's Scones", 8, 3, "24 pkgs. x 4 pieces", 10.0000, 3, 40, 5, 0
    )
    addProduct(
        db, "Gustaf's Knckebrd", 9, 5, "24 - 500 g pkgs.", 21.0000, 104, 0, 25, 0
    )
    ...
    addProduct(
        db, "NuNuCa Nu-Nougat-Creme", 11, 3, "20 - 450 g glasses", 14.0000, 76, 0, 30, 0
    )
    addProduct(db, "Gumbr Gummibrchen", 11, 3, "100 - 250 g bags", 31.2300, 15, 0, 0, 0)
    addProduct(
        db, "Schoggi Schokolade", 11, 3, "100 - 100 g pieces", 43.9000, 49, 0, 30, 0
    )
    ...
    addProduct(
        db, "Nord-Ost Matjeshering", 13, 8, "10 - 200 g glasses", 25.8900, 10, 0, 15, 0
    )
    addProduct(db, "Gorgonzola Telino", 14, 4, "12 - 100 g pkgs", 12.5000, 0, 70, 20, 0)
    addProduct(
        db, "Mascarpone Fabioli", 14, 4, "24 - 200 g pkgs.", 32.0000, 9, 40, 25, 0
    )
    ...
    addProduct(
        db, "Manjimup Dried Apples", 24, 7, "50 - 300 g pkgs.", 53.0000, 20, 0, 10, 0
    )
    ...
    addProduct(
        db, "Gnocchi di nonna Alice", 26, 5, "24 - 250 g pkgs.", 38.0000, 21, 10, 30, 0
    )
    ...
    addProduct(
        db, "Camembert Pierrot", 28, 4, "15 - 300 g rounds", 34.0000, 19, 0, 0, 0
    )
    addProduct(
        db, "Sirop d'rable", 29, 2, "24 - 500 ml bottles", 28.5000, 113, 0, 25, 0
    )
    ...
    addProduct(
        db, "Louisiana Hot Spiced Okra", 2, 2, "24 - 8 oz jars", 17.0000, 4, 100, 20, 0
    )
    addProduct(
        db,
    ...
    addProduct(
        db, "Scottish Longbreads", 8, 3, "10 boxes x 8 pieces", 12.5000, 6, 10, 15, 0
    )
    ...
    addProduct(
        db, "Mozzarella di Giovanni", 14, 4, "24 - 200 g pkgs.", 34.8000, 14, 0, 0, 0
    )
    addProduct(db, "Rd Kaviar", 17, 8, "24 - 150 g jars", 15.0000, 101, 0, 5, 0)
    addProduct(db, "Longlife Tofu", 4, 7, "5 kg pkg.", 10.0000, 4, 20, 5, 0)
    addProduct(
        db, "Rhnbru Klosterbier", 12, 1, "24 - 0.5 l bottles", 7.7500, 125, 0, 25, 0
    )
    addProduct(db, "Lakkalikri", 23, 1, "500 ml", 18.0000, 57, 0, 20, 0)
    addProduct(
        db, "Original Frankfurter grne Soe", 12, 2, "12 boxes", 13.0000, 32, 0, 15, 0
    )
